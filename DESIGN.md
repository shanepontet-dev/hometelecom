...
- **Nav items (matches the real hometelecom.co.uk top-level IA):** Broadband, Utilities, Contact us, Customer area — display-only, deliberately not wired to page sections (`href="#"`); this rebuild's own content lives at page anchors reachable by scrolling or the footer, not primary nav.
- **Dropdowns:** each nav item opens a `.nav-dropdown` panel on hover (desktop only) — an opaque white card (`shadow-md`, `radius-md`) that stays legible regardless of what's behind the header, with a `.nav-caret` chevron that rotates 180° when open. Dropdown links are inert placeholders mirroring real site structure (e.g. Customer area's dropdown repeats the footer's Customer area column verbatim). Not reachable by touch — mobile collapses to the flat 4-item list only, no nested disclosure.
